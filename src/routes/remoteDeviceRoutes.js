...
router.post('/sync-all', async (req, res) => {
    ...
    try {
        ...
        if (availableOffers) {
            try {
                // 🛡️ Robustness: Parse if string, wrap if single object
                let parsedOffers = typeof availableOffers === 'string' 
...
router.get('/device-data/:userId', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            airtimeBalance: user.deviceState?.airtimeBalance ?? 0,
            user: {
                userId: user.userId,
                ...
                remoteCommands: user.remoteCommands.slice(-10).reverse(),
            }
        });
    } catch (error) {
