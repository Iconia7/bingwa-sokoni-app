...
function getCategoryKey(planName) {
...
function getPlanEmoji(planName) {
  const lower = planName.toLowerCase();
  if (lower.includes('daily')) return '⚡';
  if (lower.includes('weekly')) return '🗂️';
  if (lower.includes('monthly')) return '🗓️';
  return '📦';
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const username = pathParts[pathParts.length - 1] || 'dita';
    if (!username) {
  ...
  useEffect(() => {
    if (activeCategory === 'All') {
      ...
      setFilteredPlans(plans.filter(p =>
        p.planName.toLowerCase().includes(activeCategory.toLowerCase())
      ));
    }
        ...
        <div className="content-wrapper">
          <div className="hero-content">
            ...
            <h1 className="hero-title">
              Data Bundles, <em>Delivered</em><br />in Seconds.
            </h1>
            <p className="hero-subtitle">
              Instant M-Pesa activation. No delays, no middlemen.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="content-wrapper">
        ...
        <div className="section-header">
          <h2 className="section-title">
            {activeCategory === 'All' ? 'Browse All Bundles' : `${activeCategory} Plans`}
          </h2>
          <span className="section-count">{filteredPlans.length} plans available</span>
        </div>

        {/* Product Grid */}
        <div className="product-grid">
          {filteredPlans.map((plan, index) => {
            const cat = getCategoryKey(plan.planName);
            const emoji = getPlanEmoji(plan.planName);
            return (
              <div
                key={plan.id}
              ...
              >
                ...
                <div className="product-body">
                  ...
                  <p className="product-offer">{plan.placeholder || 'Instant delivery to your number.'}</p>
        ...
        <div className="modal-overlay" onClick={() => !isPaying && setSelectedPlan(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-plan-badge">
              {getPlanEmoji(selectedPlan.planName)} &nbsp;{selectedPlan.planName}
            </div>
